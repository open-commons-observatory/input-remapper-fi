...
`taxonomy.yaml` is the data dictionary: facets, and for each facet its allowed values with a one-line definition. `fi db init`
(and `fi db init` again after edits) loads it into the `facet` and `facet_value` tables. **The database rejects any tag that is
...
- Every facet needs an `unknown` value where "not established" is a legitimate answer. Count them: `fi check` warns when a
  quarter or more of a facet's tags are `unknown`.
- Quote values YAML would read as booleans (`yes`, `no`, `on`, `off`); `fi` refuses non-string values.
